...
C     data for the period ahead and the period behind myTime.
     ...
     &       intime1,myIter,myThid )
        ...
        IF ( BFMcoupler_CONC{{'%02d' % i}}BotForcFile .NE. ' '  ) THEN
         CALL READ_REC_XY_RS(BFMcoupler_CONC{{'%02d' % i}}BotForcFile,CONC{{'%02d' % i}}_botF0,
     &       intime0,myIter,myThid )
         CALL READ_REC_XY_RS(BFMcoupler_CONC{{'%02d' % i}}BotForcFile,CONC{{'%02d' % i}}_botF1,
     &       intime1,myIter,myThid )
        ENDIF
        {%- endfor %}
...
C--   fill-in overlap after loading temp arrays:
        ...
        {%- for i in range(1, n_tracers + 1) %}
        _EXCH_XY_RS(CONC{{'%02d' % i}}_botF0, myThid )
        _EXCH_XY_RS(CONC{{'%02d' % i}}_botF1, myThid )
        {%- endfor %}
...
C     Adding tracers
      {%- for i in range(1, n_tracers + 1) %}
         IF ( BFMcoupler_CONC{{'%02d' % i}}BotForcFile.NE. ' '  ) THEN
           DO j=1-OLy,sNy+OLy
            DO i=1-OLx,sNx+OLx
             CONC{{'%02d' % i}}_botF(i,j,bi,bj) = bWght*CONC{{'%02d' % i}}_botF0(i,j,bi,bj)
     &                          + aWght*CONC{{'%02d' % i}}_botF1(i,j,bi,bj)
            ENDDO
           ENDDO
         ENDIF
      {% endfor %}
